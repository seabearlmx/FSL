...
def compute_mIoU( gt_dir, pred_dir, devkit_dir='', restore_from='' ):
    ...
    for ind_class in range(num_classes):
        ...
        msg = '===>'+name_classes[ind_class]+':\t' + str(round(mIoUs[ind_class]*100,2))
        logging.info(msg)
    with open(restore_from+'_mIoU.txt', 'a') as f:
        ...
        f.write('===> mIoU13: ' + str(
            round(np.mean(mIoUs[[0, 1, 2, 6, 7, 8, 10, 11, 12, 13, 15, 17, 18]]) * 100, 2)) + '\n')

    print('===> mIoU19: ' + str(round(   np.nanmean(mIoUs)*100,2   )))
    msg = '===> mIoU19: ' + str(round(   np.nanmean(mIoUs)*100,2   ))
    logging.info(msg)
